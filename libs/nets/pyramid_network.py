...
def build_heads(pyramid, ih, iw, num_classes, base_anchors, is_training=False, gt_boxes=None):
  ...
  with slim.arg_scope(arg_scope):
    with tf.variable_scope('pyramid'):
        ...
        print("##### cropped_rois.shape", cropped_rois.shape)
        outputs['roi']['cropped_rois'] = cropped_rois    #shape = [?, 14, 14, 256]
        ...
        print("##### final_boxes.shape", final_boxes.shape)    #(?, 4)
        print("##### classes.shape", classes.shape)            # <unknown>
        print("##### cls2_prob.shape", cls2_prob.shape)        #(?, 81)
...
def build_losses(pyramid, outputs, gt_boxes, gt_masks,
  ...
  gt_masks: A tensor of shape (G, ih, iw),  {0, 1}蘙Ma蘙Ma蘛]
...
def build(end_points, image_height, image_width, pyramid_map, 
    ...
    # print("build_heads outputs.shape", outputs.shape)
    ...
    print("##### outputs['mask']['mask'].shape", outputs['mask']['mask'].shape)    #(?, 28, 28, 81)
    first_mask = outputs['mask']['mask'][:1]
    print("##### first_mask.shape", first_mask.shape)  # (?, 28, 28, 81)
